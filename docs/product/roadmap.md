...
| R-J17 | **A file opened as a sliver under its own toolbar** — reported 2026-08-07: *"the editor now opens but the content is not fully expanded to the size of the pane, I can barely see the content"*. One class. The root asked for `flex-1`, and `ZoomPane` — the wrapper every pane body is mounted in — is a positioned **block**, so there was no flex parent to answer it: the root sized to its content, and the editor under it took its share of a height that was never distributed. **It is a fossil of `R-B53`**, which lifted this markup out of the Code pane's flex column to be the pane root and carried the child's class up with it; `flex-1` was correct in the old position and inert in the new one, which is why nothing warned. Every other pane root says `h-full`, and this one does now. The guard is honest about its own weakness: jsdom does no layout and no Tailwind, so it cannot measure a height — `clientHeight` would have passed against the bug — and instead mounts through the real `ZoomPane` and pins the contract the bug broke | S | ⏳ |
